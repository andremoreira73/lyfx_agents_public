...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            new_jobs_count, duplicates_skipped = self.save_results(scan_run, results, options['dry_run'])
            
            # Update scan run with actual counts from database
            if not options['dry_run']:
                # Get real counts from what was actually saved
                actual_jobs = JobPosting.objects.filter(scan_run=scan_run)
                scan_run.total_jobs_found = actual_jobs.count()
                scan_run.total_top_picks = actual_jobs.filter(is_top_pick=True).count()
            ...
            if new_jobs_count == 0:
                self.stdout.write(
                    self.style.WARNING(
                        f'No new jobs found. Skipped {duplicates_skipped} duplicates. '
                        f'Total processed: {len(results.get("processed_urls_ll", []))} pages'
                    )
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Successfully saved {new_jobs_count} new jobs. '
    ...
    def save_results(self, scan_run, results, dry_run=False):
        ...
        return new_jobs_count, duplicates_skipped
